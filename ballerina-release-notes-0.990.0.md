...
Ballerina 0.990.0 consists of significant improvements of language syntax which is based on a new specification.
...
The anydata type consists of pure values whose basic type is not error. Thus (anydata|error) contains all pure values. Thus anydata is equivalent to () | int | float | decimal | string | (anydata|error)[] | map<anydata|error> | xml | table

Ballerina records are by default open. In the previous release record {} mean  record { any … }, with the introduction of pure values now record {} means it is open only to pure values. 

### clone() built-in method

Performs a deep copy of a pure value. It is a no-op operation on simple values and error values. This operation is guaranteed to succeed on pure values and preserves cycles, inherent type, immutability of values. 
...
From this release onwards, we’ve changed the semantics of operator ‘==’ to perform a deep value equality only on anydata values, compile time error otherwise. This operator is not allowed on error values.

We’ve introduced a new operator ‘===’ that performs reference equality on reference types and is otherwise same as ‘==’. The !== operator results in the negation of the result of the === operator. 
...
There is a  isFrozen() built-in method on all structural basic types returns true or false depending on whether the value is frozen. 
...
Ballerina distinguishes two kinds of errors; normal and abnormal errors. Abnormal errors do not happen normally and they are usually signs of bugs or programmer errors. It is usually not possible to recover from abnormal errors. Normal errors, on the other hand, are recoverable and they are part of your business logic. 
...
The any type was the union of all types in Ballerina. But from this release onwards, we’ve removed the error type from any. This change forces errors to be documented explicitly, even if a function returns any. 

### check expression

The check is a useful unary operator in the language which allows you to get an error out of the way in arbitrary expressions. If the type of the expression E is T | error, then the type of expression check E will be T if it evaluates to T. If the expression evaluates to error, then the function terminates immediately as if by “return E”. The check operator forces you to declare error as part of the return type of your function. 

In the previous release, check throws the error if the function does not have error type as part of the return type. This causes normal errors to become exceptions or abnormal errors due to laziness of the programmer. 
...
This is a replacement for “throw” to allow programmers to create a panic which causes the execution to stop and function calls will unwind until the panic is stopped by trapping it and converting it into an error. Stack trace is recorded in the error value at the time of panic. 
...
Tuple binding pattern destructures a tuple value based on the number of members in the tuple value.
Here is an example of a tuple binding pattern.
...
Record binding pattern destructures a mapping value based on the presence of a field name. 
...
Ballerina has built in support for concurrency.  `worker` and `start` creates new concurrent flows. `wait`, `->` (send), `<-` (receive), ‘flush’ constructs offer flow control and communication between concurrent flows. `lock` construct can be used to achieve safe concurrent access.
...
A function has one or more workers including unnamed default worker and other named workers. These workers  are peers of each other. The termination of the function is independent of the termination of its named workers. Returning from a named worker terminates that worker, but does not cause a return from the function. Workers have same return declaration as functions which defaults to ()
...
A wait-action waits for one or more futures to terminate and makes available their return value (including any raised errors).  Panic in waited-for future should result in panic in `wait`. Waiting for a future and waiting for a worker are conceptually the same. 
...
If the data is not available, receive will wait, similarly if the receiver is not reached yet, the sync send will wait. Both sync send and receive will propagate which ever the error happened in the opposing strand to the current strand.

`flush` will wait until all the message are send from current worker to the specified worker. If the receiver failed or panicked, then that error will be propagated to the waiting strand.
...
oncommit, onabort function handlers are removed from transaction statement and are substituted with committed, aborted blocks.
...
Above code will cause calling transaction to fail due to the panicking of a participant.
...
oncommit, onabort functions should have the signature of 'function (string trxId)'
...
And panic in remote participant will result in transaction failure.
...
TCP server and client completely redesigned with this release. Now user can use the TCP server similar to the WebSocket listener where there are set of predefined resources to invoke based on actions that take place. TCP client can be use to connect to a remote TCP server for data sending purpose. Additionally callback service can be attached to the client if there is a data retrieve use case.
...
Note: Listener endpoint port is the first argument and other listener configuration goes under second argument. 
...
Note: Server url is the first argument and other endpoint configuration goes under second argument.

We introduce contract first approach for gRPC service creation which means you first define the service contract(.proto file). From the service contract, you generate stub and sample Ballerina service using the built-in proto compiler. Earlier we follow code first approach to create gRPC service, but we depreciated it due to a limitation in ensuring backward compatibility. Instead we recommend contract first approach to create gRPC service.
...
Improvements to the WebSub Hub start up function
Parameters accepted by the hub start up function are categorized into two categories. The hub start up function now accepts an http:Listener, on which the hub service will start on, as the first parameter, and a HubListenerConfiguration record as a defaultable parameter to specify hub related configuration.  
...
New syntax for JMS message producer and message receiver are as below. 
...
Added test generation code action for service and functions
...
Language Core introduces experimental features and in order to support code intelligence such as auto completion in VSCode Plugin, user have to enable the `Allow Experimental` setting in the user settings.
...
ballerinax/kubernetes module now supports generating Istio gateways and Istio virtual services for ballerina services. These artifacts can be generated with @kubernetes:IstioGateway and @kubernetes:IstioVirtualService annotations which allow to deploy services seamlessly to Istio.
...
Please refer [Github milestone issues](https://github.com/ballerina-platform/ballerina-lang/issues?q=is%3Aissue+milestone%3A0.990.0+is%3Aclosed+label%3AType%2FBug) to view bug fixes
